...
public class GiftAction implements GiftActionService {
    ...
    public void giveChlebeek(Player player) { //easter egg
        player.getInventory().addItem(new ItemBuilder(Material.BREAD).setDisplayName(ChatColor.GOLD + "Chleb").build());
    }

    @Override
    public void giveEnderpearl(Player player) {
        player.getInventory().addItem(new ItemBuilder(Material.ENDER_PEARL).setDisplayName(ChatColor.AQUA + "Perelka").build());
    }
    ...
    public void gamemode(Player player) {
        ...
        }.runTaskLater(McTiktokGiftActions.getMcTiktok(), 20L * 10L);
    }
